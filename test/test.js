...
contract('dBank', ([deployer, user]) => {
  ...
  beforeEach(async () => {
    ...
    await token.reAssignMinterRole(dbank.address, {from: deployer})
  })

  describe('testing token contract...', () => {
    describe('success', () => {
      it('checking token name', async () => {
        expect(await token.name()).to.be.eq('Bank of Sherlock')
      })

      it('checking token symbol', async () => {
        expect(await token.symbol()).to.be.eq('BOS')
      })
    ...
    describe('failure', () => {
      it('passing minter role should be rejected', async () => {
        await token.reAssignMinterRole(user, {from: deployer}).should.be.rejectedWith(EVM_REVERT)
      })
